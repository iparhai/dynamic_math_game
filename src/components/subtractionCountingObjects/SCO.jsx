...
export default function SCO(props) {
    ...
    useEffect(() => {
        setproblem(problemGenerator.generateSubtractionProblem("", 3))
        setObject(getRandomObject(props.objectList))
        setanswer(0)
    }, [])
    const evaluateProblem = () => {
        setIfCorrect(problemSolver.solveSubtractionProblem(problem, answer))
    }
    useEffect(() => {
    ...
    return (
        <div>
            <h1 style={{ fontSize: "3.5em" }}> {problem} </h1>
